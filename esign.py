...
class LivePDFESign:
    def __init__(self, root):
        ...
        self.pdf_path = None
        self.current_page = 0
        self.signatures = []  # list of dicts {text, type, x, y, size, color, page}
        ...
        self.drag_update_interval = 16  # ms ~60 FPS
    ...
    def show_welcome_message(self):
        ...
        # Modern welcome screen with gradient-like background
    ...
    def open_pdf(self):
        ...
        try:
            ...
            self.status_var.set(f"📄 Loaded: {filename} ({len(self.pdf_doc)} pages) - Ready for professional signing!")
    ...
    def draw_signature_on_canvas(self, sig, index):
        ...
        # Enhanced visual styling with better shadows and backgrounds
        ...
        if bbox:
            ...
            # Modern background with rounded corners effect
            padding = 6
    ...
    def add_signature_at_center(self, text, sig_type):
        ...
        # Use fixed sizes and colors (no customization)
    ...
    def hex_to_rgb01(self, color_str):
        """Convert #RRGGBB to (r,g,b) floats in 0..1 for PyMuPDF."""
        if not color_str:
            return (0, 0, 0)
        s = color_str.strip()
        if s.startswith("#"):
    ...
    def save_pdf(self):
        ...
        try:
            # Work on a copy of the original document
            doc = fitz.open(self.pdf_path) if self.pdf_path else self.pdf_doc
            
            # Insert texts
            for i, sig in enumerate(self.signatures):
                page = doc[sig["page"]]
                # x, y are already stored in page coordinates in points
                fontsize = float(sig["size"])  # in points for insert_text
                color = self.hex_to_rgb01(sig["color"])  # tuple of floats 0..1
